...
public abstract class BaseDiscountAccountResourceTestCase {
	...
	public void testGetDiscountByExternalReferenceCodeDiscountAccountsPage()
		...
		assertContains(
			discountAccount1, (List<DiscountAccount>)page.getItems());
		assertContains(
	...
	public void testGetDiscountByExternalReferenceCodeDiscountAccountsPageWithPagination()
		...
		if (totalCount >= (pageSizeLimit - 2)) {
			...
			assertContains(
				discountAccount1, (List<DiscountAccount>)page1.getItems());
			...
			assertContains(
				discountAccount2, (List<DiscountAccount>)page2.getItems());
			...
			assertContains(
				discountAccount3, (List<DiscountAccount>)page3.getItems());
		}
		else {
			Page<DiscountAccount> page1 =
				discountAccountResource.
			...
			Assert.assertEquals(
				discountAccounts1.toString(), totalCount + 2,
				discountAccounts1.size());
			...
			assertContains(
				discountAccount1, (List<DiscountAccount>)page3.getItems());
			assertContains(
				discountAccount2, (List<DiscountAccount>)page3.getItems());
			assertContains(
				discountAccount3, (List<DiscountAccount>)page3.getItems());
		}
	}
	...
	public void testGetDiscountIdDiscountAccountsPage() throws Exception {
		...
		if (irrelevantId != null) {
			...
			assertValid(
				page,
				testGetDiscountIdDiscountAccountsPage_getExpectedActions(
					irrelevantId));
		}
	...
	protected Map<String, Map<String, String>>
			testGetDiscountIdDiscountAccountsPage_getExpectedActions(Long id)
		throws Exception {
	...
	@Test
	public void testGetDiscountIdDiscountAccountsPageWithSortDouble()
		throws Exception {
		...
		testGetDiscountIdDiscountAccountsPageWithSort(
			...
			(entityField, discountAccount1, discountAccount2) -> {
				BeanTestUtil.setProperty(
					discountAccount1, entityField.getName(), 0.1);
				BeanTestUtil.setProperty(
					discountAccount2, entityField.getName(), 0.5);
			});
	}
	...
	protected void testGetDiscountIdDiscountAccountsPageWithSort(
		...
		for (EntityField entityField : entityFields) {
			...
			assertContains(
				discountAccount1, (List<DiscountAccount>)ascPage.getItems());
			assertContains(
				discountAccount2, (List<DiscountAccount>)ascPage.getItems());
			...
			assertContains(
				discountAccount2, (List<DiscountAccount>)descPage.getItems());
			assertContains(
				discountAccount1, (List<DiscountAccount>)descPage.getItems());
		}
	}
	...
	protected void assertContains(
		...
		for (DiscountAccount item : discountAccounts) {
			if (equals(discountAccount, item)) {
				contains = true;
		...
		Assert.assertTrue(
			discountAccounts + " does not contain " + discountAccount,
			contains);
	}
	...
	private static final com.liferay.portal.kernel.log.Log _log =
		LogFactoryUtil.getLog(BaseDiscountAccountResourceTestCase.class);
