...
public abstract class BaseGroupResourceTestCase {
	...
	public void testEscapeRegexInStringFields() throws Exception {
		...
		group.setDisplayName(regex);
		group.setExternalId(regex);
		group.setId(regex);
		...
		Assert.assertEquals(regex, group.getDisplayName());
		Assert.assertEquals(regex, group.getExternalId());
		Assert.assertEquals(regex, group.getId());
	}
	...
	protected void assertValid(
		Page<Group> page, Map<String, Map<String, String>> expectedActions) {
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("displayName", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						group1.getDisplayName(), group2.getDisplayName())) {
			...
			if (Objects.equals("externalId", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						group1.getExternalId(), group2.getExternalId())) {
			...
			if (Objects.equals("members", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						group1.getMembers(), group2.getMembers())) {
			...
			if (Objects.equals("meta", additionalAssertFieldName)) {
				if (!Objects.deepEquals(group1.getMeta(), group2.getMeta())) {
					return false;
				}
			...
			if (Objects.equals("schemas", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						group1.getSchemas(), group2.getSchemas())) {
	...
	protected String getFilterString(
		...
		if (entityFieldName.equals("externalId")) {
			Object object = group.getExternalId();
