...
export class TestEntityApi {
	 ...
	 * 
	 */
	public async getTestEntitiesPage (options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PageTestEntity; }> {
		...
		return interceptorPromise.then(() => {
			...
			return new Promise<{ response: http.IncomingMessage; body: PageTestEntity; }>((resolve, reject) => {
	 ...
	 * @param testEntityId 
	 */
	public async getTestEntity (testEntityId: number, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: TestEntity; }> {
		const localVarPath = this.basePath + '/test/v1.0/test-entities/{testEntityId}'.replace('{' + 'testEntityId' + '}', encodeURIComponent(String(testEntityId)));
		let localVarQueryParameters: any = {};
		let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
		const produces = ['application/json', 'application/xml'];
		// give precedence to 'application/json'
		...
		return interceptorPromise.then(() => {
			...
			return new Promise<{ response: http.IncomingMessage; body: TestEntity; }>((resolve, reject) => {
				localVarRequest(localVarRequestOptions, (error, response, body) => {
					if (error) {
						reject(error);
					} else {
	 ...
	 * Retrieves the count.
	 */
	public async getTestEntityCount (options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: number; }> {
	 ...
	 * @param body 
	 */
	public async postReservedWord (body?: boolean, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body?: any; }> {
